  var ipArduino = 'http://186.220.99.147:8099';
        ...
        if (window.openDatabase) {
            ...
            if (!db) {
                ...
                db.transaction(function (tx) {
                    tx.executeSql('CREATE TABLE IF NOT EXISTS DataLog (Data, Temperatura, PH)');
                });
            }
        }
        else {
    ...
    function GravaLog() {
        ...
        if (window.openDatabase) {
            ...
            if (db) {
                db.transaction(function (tx) {
                    tx.executeSql("INSERT INTO DataLog (Data, Temperatura, PH) values(?, ?, ?)", [[new Date().getTime(), '1', '2']], function(tx, e) {
		  ...
		  if (window.openDatabase) {
            ...
            if (db) {
                db.transaction(function (tx) {
                    tx.executeSql("INSERT INTO DataLog (Data, Temperatura, PH) values(?, ?, ?)", [[new Date().getTime(), dadosRecebidos.Temp, dadosRecebidos.PH]], function(tx, e) {
					  alert("There has been an error: " + e.message);
					},  function(tx, r) {
					  // re-render the data.
					  console.log("Log gravado com sucesso!");
            ...
            if (!db) {
                ...
                db.transaction(function (transaction) {
                    transaction.executeSql(
                        "SELECT top 20 * FROM DataLog order by Data desc ",
                        [[]],
                        function (transaction, result) {
                            ...
                            for (var i = 0; i < result.rows.length; i++) {
                                ...
                                console.log(result.rows.item(i)[['Temp']]);
